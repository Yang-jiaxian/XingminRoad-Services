...
from src.api.v1.contact.schemas import CreateContactParams, UpdateContactParams, CreateContactResp, FetchContactsResp
...
@contact_app.post("/contacts", summary="新增联系记录", response_model=CreateContactResp)
async def create_contact_api(
    ...
    return output_json(data={"contact_id": contact_id}, message="")


@contact_app.delete("/contacts/{contactId}", summary="删除联系记录", response_model=CreateContactResp)
...
async def fetch_contacts_api(
    ...
    return output_json(data=contacts, total=total, message="")
