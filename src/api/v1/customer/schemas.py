...
class CreateCustomerParams(BaseModel):
    ...
    customer_type: CustomerType = Field(..., title="客户类型",
    ...
    existing_assets: float = Field(None, title="资产-现有资产")
    historical_peak: float = Field(None, title="资产-历史峰值")
    ...
    scale_of_management: Optional[ScaleOfManagement] = Field(None, title="管理规模")
    private_placement_strategy: Optional[PrivatePlacementStrategy] = Field(None, title="私募的策略类型")
    fund_demand: Optional[FundDemand] = Field(None, title="资金需求")
    technical_demand: Optional[TechnicalDemand] = Field(None, title="技术需求")
    bond_source_demand: Optional[BondSourceDemand] = Field(None, title="券源需求")
    investment_research_demand: Optional[InvestmentResearchDemand] = Field(None, title="投研需求")
